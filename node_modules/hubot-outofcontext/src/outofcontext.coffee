...
giangiQuote = (randomQuote, msg) ->
  quote = ["Ma l'altro giorno avevi detto \"#{randomQuote}\" ,#{msg.message.user.name}",
	   "O come dice #{msg.message.user.name} , \"#{randomQuote}\" hahahahahaha che ridere",
	   "L'hai detto tu \"#{randomQuote}\" eheheheh che burlone",
	   "Mi serve un po' di relax, #{msg.message.user.name}, mi serve un po' di relax!"]
...
module.exports = (robot) ->
  ...
  robot.hear /(.*)/i, (msg) ->
    ...
    if Math.floor(Math.random() * 25) == 2
      appendQuote(robot.brain.data.oocQuotes, user, msg.match[1])
      
  robot.hear /giang|certo certo|esatto/i, (msg) ->
    return unless robot.brain.data.oocQuotes?
    user = userId(robot, msg.message.user.name)
    if (quotes = robot.brain.data.oocQuotes[user])
      randomQuote = quotes[Math.floor(Math.random() * quotes.length)]
